...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (!window.supabase) {
        console.error("CRITICAL ERROR: Supabase client is missing. Ensure CDN is loaded before config.");
        return;
    }
    ...
    async function loadModelData() {
        try {
            ...
            // FIX: Using maybeSingle() instead of single() to prevent crashes when 0 rows are found
            ...
            if (avatarTextEl) {
                avatarTextEl.textContent = model.avatar_text || model.model_name.substring(0, 2).toUpperCase();
            }
